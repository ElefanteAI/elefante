...
**Compendiums and pitfall reference for Elefante v2.6.0**
...
| BUG-015 | GitHub Release publish failure — the `Build One-Click Binaries` matrix can now succeed on `ubuntu-latest`, `macos-latest`, and `windows-latest`, yet the downstream `Create GitHub Release` job still fails after artifacts exist. Available evidence narrows the fault to the release stage, not binary compilation. | OPEN — root cause not yet evidenced | [ops-installation #9](ops-installation-compendium.md#issue-9-github-release-publish-failure-after-successful-matrix-builds) | Manual: run `Build One-Click Binaries` on a `v*` tag and confirm all three build jobs AND `Create GitHub Release` finish green with assets attached to the release | 1x — warning email after BUG-014 |
...
_Last verified: 2026-04-15 | Elefante v2.6.0_
