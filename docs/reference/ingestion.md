...
entities and metadata, plus `force_new`. A successful write requires a memory
search earlier in the same MCP session; that compliance gate is enforced by the
server before the orchestrator runs.
...
3. **Check related knowledge.** Preference reassertions may merge into a close
   ...
   related, redundant, or contradictory. Contradiction detection is heuristic,
   not proof that one statement is correct.
...
ordinary memories automatically because they are old, nor does it implement
user locks or mandatory injection policies. Those governance capabilities must
remain documented as development work until the runtime ships them.
