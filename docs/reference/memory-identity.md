...
1. `MemoryAdd` computes `canonical_key` once at write. If a non-deprecated memory exists with the same key, the new write either updates that record or is marked `REDUNDANT` against it. No silent duplicate.
...
- A test asserts that two `MemoryAdd` calls with the same canonical_key produce one active memory, not two.
