...
If your Docker environment can reach GitHub, clone and check out the current
released tag shown in the commands below. The tag is release-managed; `main` is
a development branch and is not the customer release contract.
...
For a portable memory migration, the bundle also carries
`scripts/pipeline/import_memories.py`. It is dry-run-first, regenerates vectors
with the configured local model, and does not restore graph topology. See
`docs/how-to/rollback.md` in the full checkout for the backup gate.
