...
1. **Pre-Flight Checks** (automated safeguards)
   ...
   - See [`workspace/postmortems/installation.md`](../../workspace/postmortems/installation.md) for details
...
**Solution**: This is a known bug fixed in current code. Ensure you are running the latest version — `src/utils/elefante_mode.py` must have the `sys.platform != "win32"` guard around `import fcntl`. See `workspace/postmortems/installation.md` → `pitfall: installation fcntl windows incompatibility`.
...
**Solution**: See [`workspace/postmortems/installation.md`](../../workspace/postmortems/installation.md) — search `pitfall: installation kuzu`
...
1. Check [`workspace/postmortems/installation.md`](../../workspace/postmortems/installation.md) for common installation pitfalls
...
1. **Explore the API**: [`tools.md`](../reference/tools.md)
2. **Try the Dashboard**: [`view-dashboard.md`](view-dashboard.md)
3. **Understand Architecture**: [`architecture.md`](../reference/architecture.md)
...
1. **Elefante exists** — call `elefante-Memory(action="search")` before answering questions about preferences, decisions, or conventions
...
- `elefante-Memory(action="add")`: Score is system-computed; classify `memory_type` accurately
- `elefante-Memory(action="search")`: Search bias warnings; contradiction resolution rules
...
1. **Explicitly** — via `elefante-Memory(action="search")` (agent calls it proactively, triggered by Layer 1 instructions)
...
[Layer 1] VS Code loads .github/copilot-instructions.md
       ...
       |
       v
Agent calls elefante-Memory(action="search") (or any Elefante tool)
       |
       v
