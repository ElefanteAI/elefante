...
- **Compliance Gate (search-before-write)** is enforced:
  - You MUST call `elefante-Memory(action="search")` before any write tool.
- **Actionable Compliance**: You MUST obey the `suggested_action` header injected into the payload of every `elefante-Memory(action="search")` result.
...
- Writes blocked: you forgot to call `elefante-Memory(action="search")` first.
...
**Command:** Attempt to call `elefante-Memory(action="add")` with a new "dummy" memory BEFORE you have performed a search in this session.
...
1. Call `elefante-Memory(action="search")` for "Jaime's output preferences".
2. Based on the result, perform a `verify_health.py` and save a new memory using `elefante-Memory(action="add")` that summarizes the health status while following the output preferences.
