...
Budgets are linked to memory type half-lives from `spec-memory-schema.md`:
...
`elefante-MemoryAdd` responses include:
...
| `docs/technical/spec-tools.md` | **MODIFIED** — TOKEN_STATS in tool response contract | ~15 |
| `docs/technical/spec-architecture.md` | **MODIFIED** — Token Intelligence Layer section | ~15 |
| `docs/planning/spec-vision.md` | **MODIFIED** — Shipped table + tool count fix | ~3 |
...
Performed per `docs/technical/dev-sdd.md` Gate 2:
...
| 9 | TOKEN_STATS documented in `spec-tools.md` | Yes — tool response contract section updated |
| 10 | Token Intelligence Layer documented in `spec-architecture.md` | Yes — new section added |
