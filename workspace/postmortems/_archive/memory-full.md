...
> **Status:** Production Reference - All Scoring Flaws Fixed (v2.7.0); GAP-025 documented, closure pending
> **Applies to**: v2.9.3+
...
- [Issue #15: Multi-Instance Write Origin Tracking — No Source Attribution, Concurrent-Writer Data Risk](#issue-15-multi-instance-write-origin-tracking--no-source-attribution-concurrent-writer-data-risk) DOCUMENTED
...
`elefante-MemoryAdd` returns `success: false`. The error message cites `workspace/ISSUES.md -> Known Issues` but no matching BUG row exists.
...
## Issue #15: Multi-Instance Write Origin Tracking — No Source Attribution, Concurrent-Writer Data Risk

**Date:** 2026-04-18
**Duration:** Discovery via architectural audit prior to Phase 2 installer work
**Severity:** HIGH — latent data-integrity risk; blocks session-intelligence and cross-IDE reach
**Status:** DOCUMENTED — GAP-025 filed; closure planned via [`spec-ide-integration-surface.md`](../planning/spec-ide-integration-surface.md) and [`spec-session-intelligence.md`](../planning/spec-session-intelligence.md) § 6.7

### Problem

Two compounding absences in Elefante's write path make multi-instance use unsafe today:

1. **No origin metadata on any memory.** A memory written by Claude Code from project `~/proj-a` and a memory written by Cursor from project `~/proj-b` are indistinguishable in the store. There is no `tool`, no `instance_id`, no `cwd`, no `session_id` carried onto the write.
2. **No concurrency model for two live clients.** Each IDE spawns its own stdio MCP subprocess, so two running IDEs mean two processes attempting to write the same Kuzu graph file. Kuzu does not tolerate concurrent writers.

These two gaps together create a data-corruption path: the second stdio process either errors out (user sees "Elefante is broken") or writes through a stale handle (user gets silent graph corruption). In either case there is no Source tuple to reconstruct which IDE owned which write.

### Symptom

- A user asks "which IDE wrote this memory?" — Elefante cannot answer. The field does not exist.
- A user opens Claude Code and Cursor on the same repo simultaneously; one of them fails to start the MCP server with a Kuzu lock error, or both start and the graph file drifts out of sync with ChromaDB.
- A memory written from project A surfaces in an unrelated project B's retrievals with no way to understand why, because the write had no `cwd`.
- Session-intelligence work (see [`spec-session-intelligence.md`](../planning/spec-session-intelligence.md) § 6.2) cannot attribute events to clients, because the attribute channel does not exist upstream.

### Root Cause

Three structural absences, each sufficient on its own to block the product contract, compound in the current architecture:

**(a) Storage schema gap.** `MemoryModel.metadata` has 40+ fields but no `source.*` substructure. Nothing at the schema layer invites the writer to declare its origin.

**(b) Transport architecture gap.** The documented MCP transport is stdio (see [`docs/technical/`](../technical/) and existing IDE configs). stdio forces one subprocess per client; the DB files are therefore multi-writer by construction when multiple IDEs run. Kuzu's single-writer contract (see `ops-database-compendium.md` Issue #2) is violated by default.

**(c) Source attribution gap.** Even where `session_id` exists as a tool argument (see `spec-session-intelligence.md` § 2), the server does not normalize or persist it, and there is no `client_name`, `instance_id`, or `cwd` captured alongside. The write path is effectively anonymous.

### Solution

Closure requires three coordinated changes, tracked as GAP-025 and executed across two specs:

1. **Introduce the Source schema** in the storage layer (graph-side):

   ```
   (:Memory)-[:WRITTEN_BY]->(:Source {
     tool: string,            // normalized matrix id: claude-code, cursor, bob, ...
     instance_id: uuid,       // per IDE window/process, generated on MCP handshake
     session_id: string,      // server-generated, per spec-session-intelligence.md § 6.1
     cwd: string,             // working directory at write time
     matrix_version: string,  // pinned version of ide-integration-matrix.yaml
     timestamp_utc: iso8601
   })
   ```

   Source nodes deduplicated on the `(tool, instance_id, session_id)` tuple. Detailed in [`spec-session-intelligence.md`](../planning/spec-session-intelligence.md) § 6.7.

2. **Replace stdio-per-client with a singleton daemon** (launchd/systemd user-scope) owning single-writer handles on ChromaDB and Kuzu, exposing MCP over streamable-http on `127.0.0.1:<port>`. All IDE configs point at the same endpoint. Detailed in [`spec-ide-integration-surface.md`](../planning/spec-ide-integration-surface.md) § 4.2 and § 7.1.

3. **Capture origin at the MCP handshake.** The daemon records `tool` from the handshake envelope, generates `instance_id` per connection, issues `session_id` on session start, and carries `cwd` on each invocation. Those values are stamped onto every write.

### Verification Command

Phase B of [`spec-ide-integration-surface.md`](../planning/spec-ide-integration-surface.md) defines the acceptance test:

```
# With daemon running and two IDEs connected:
# 1. Issue a MemoryAdd from IDE A, then one from IDE B.
# 2. Query the graph:
#    MATCH (m:Memory)-[:WRITTEN_BY]->(s:Source) RETURN m.id, s.tool, s.instance_id, s.cwd
# 3. Confirm two distinct instance_id values, matching tool names, and real cwd paths.
# 4. Confirm no Kuzu lock errors in the daemon log during the two concurrent writes.
```

### Broader Risk

- Every future product question of the form "which client / which project / which session produced this memory?" is unanswerable until the Source schema lands. That includes all of [`spec-session-intelligence.md`](../planning/spec-session-intelligence.md) § 6.3 (invocation event log), much of [`spec-retrieval-effectiveness.md`](../planning/spec-retrieval-effectiveness.md) (per-project reuse signal), and any future per-client telemetry.
- Every minute the system runs without the singleton daemon is a minute where two-IDE use is a graph-corruption dice roll. The user has no visibility into which throws hit; silent corruption may only surface weeks later when retrieval results become unexplainable.
- The `integration-inspector` agent ([`agents/integration-inspector.md`](../../agents/integration-inspector.md)) is downstream of this closure — it cannot flag "matrix row writes to a dead path" with confidence until the Source tuple distinguishes which surface the failed write came from.

### Why This Took So Long To Surface

The v2.x storage model was built for a single-user, single-IDE workflow. As long as only one IDE was open, stdio-per-client worked and the missing Source tuple was invisible (the owner was implied by "the one running IDE"). The gap only became a first-class bug when the cross-IDE installer work (Phase 2 of the installer, [`spec-ide-integration-surface.md`](../planning/spec-ide-integration-surface.md)) forced the question: "what happens with two live clients?" — at which point it was immediately visible that neither the storage schema nor the transport architecture was ready for the answer.

### Lesson

> **Any system that writes user data must capture who is writing, from where, in which session — structurally, not aspirationally. Origin is not a nice-to-have logging field; it is the foundation of every downstream attribution question. Treat it as a first-class schema concern, not a telemetry afterthought.**
