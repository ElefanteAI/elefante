...
> No accepted ADRs exist yet. Current decisions remain in the living plan and
> linked postmortems until a separately approved migration occurs.
...
| BUG-016 (domain signal removal) — `workspace/postmortems/memory.md` Issue #11 | `ADR-0007-removed-domain-signal.md` |
| BUG-017 (intent-gated spec override) — `workspace/postmortems/memory.md` Issue #12 | `ADR-0008-intent-gated-spec-override.md` |
| BUG-018 (co-activation persistence) — `workspace/postmortems/memory.md` Issue #13 | `ADR-0009-co-activation-persistence.md` |
...
This inventory has no assigned release or date. It is not an implementation
commitment.

## Authority

When an ADR is authored, it becomes the source of truth for that decision.
Existing homes (`workspace/PLANNING.md §2.5`, `workspace/lessons.md`, and linked
postmortems) get replaced with cross-references to the new ADR, not duplicates.
