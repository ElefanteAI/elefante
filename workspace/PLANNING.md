...
|------|-------|--------|-------------|
| 2026-08-26 | **Surgical reattachment of this Codex task's stale Elefante bridge was authorized and frozen before execution.** Scope is limited to a graceful termination of stdio bridge PID `24229`, the eight-hour-old child of the active Codex app-server PID `22073`; the healthy daemon PID `55811`, durable data, installed runtime, host configuration, repositories, and remote surfaces are excluded. | The repaired candidate is healthy through its own doctor and a new direct bridge, but this task reproduced HTTP 404 through the bridge session created before installation. OpenAI's documented client action is MCP Restart; recycling only the owned stdio child is the smallest local equivalent available without quitting Codex. | Preflight at `2026-08-26T17:34:15-04:00`: installed v2.12.3 candidate at clean source `8b7cc5b` reports 17 tools, read-only Recall, `status=supplied`, and `customer_ready=true`; daemon health is green; this task's real Recall returns HTTP 404. Public `origin/main` is `14fda301`, PR #25 remains open/draft/DIRTY at `d9aefb1e`, the original 49-path dirty checkout fingerprint remains `cd285fad...8df`, and website source/live remain `d4e1f321` at v2.12.3. Execute one bounded `TERM`, require a replacement bridge attached to the same Codex host, then require one real read-only Recall event. Stop without broader restart if reattachment fails or any frozen fact changes. No outcome trial, lift claim, memory mutation, daemon restart, push, PR update, merge, release, or deployment is authorized. |
