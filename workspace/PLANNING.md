...
| Task Intelligence Pipeline (bounded Task Briefs + controlled outcome evaluation) | [`workspace/proposals/retrieval-effectiveness.md`](../workspace/proposals/retrieval-effectiveness.md) | V2 shadow and three black-box canaries implemented; no correctness lift demonstrated; 27 tasks and default injection remain blocked pending a valid causal benchmark |
...
|------|-------|--------|-------------|
| 2026-08-06 | **Task Intelligence black-box canaries repaired the judge but did not establish lift.** Three CLI/API/filesystem fixtures each fail at the exact base ref and pass at the exact known-good ref. The evaluator now excludes user/project instruction noise, aborts zero-token CLI failures, uses short disposable paths, records unmeasured retry data as unknown, and supports disclosed treatment-only memories. | A valid judge, relevant retrieval, and delivered memory are necessary but individually insufficient. The restore treatment received its disclosed safety procedure and still missed the accepted behavior, so promotion cannot be inferred from delivery or lower cost. | Current retrieval diagnostic: 16/18 top-ten repair-path hits. CORS: baseline 3/3, treatment 3/3. Restore: no lift after confirmed memory delivery. Focused suite: 51 passed. Three tasks are contract-valid; 27 remain ineligible; V1 stays default and no public surface or claim is authorized. Next: add stage-level failure observability and independently reviewed black-box tasks before more paired model runs. |
