...
|----------|------------------|---------------|-----------|
| **P0 — Improve one real memory-dependent task** | The governed memory bundle causes a better accepted answer/action than the source-only Task Brief v2 path | Task 032 is a completed local `STOP`: intended memory selected/delivered 3/3, treatment accepted 0/3, control 0/2 before irreversible early stop. All five patches missed the public Recall MCP tool. The memory was broadly relevant but not task-locally discriminative. | Select a different task only when one prior memory adds a specific decision-relevant fact absent from the frozen source Brief; prove that difference model-free before product changes or model spend. |
...
| BUG/GAP count tracked | **49 BUG records through BUG-051 + 4 GAPs** | `workspace/ISSUES.md` | Tracked |
...
| Full fast regression suite | 373 passed, 4 legacy-backend skips, 1 slow test deselected | §10 documentation-audit journal entry | Verified 2026-08-06 |
| Task Intelligence evaluation corpus | 8 reviewed black-box canaries; 23 historical tasks ineligible; one sealed real-memory preflight | `workspace/proposals/retrieval-effectiveness.md` | Infrastructure verified; promotion blocked |
| Task Intelligence outcome lift | One preserved real-memory treatment passes its corrected one-task judge; no valid paired multi-task lift exists | `workspace/proposals/retrieval-effectiveness.md` | Functional signal only; promotion blocked |
