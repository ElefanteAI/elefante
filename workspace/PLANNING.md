...
### §3.4 Priority order after v2.12.2
...
|----------|------------------|---------------|-----------|
| **P0 — Improve one real memory-dependent task** | The governed memory bundle increases accepted task value per total token against the source-only Task Brief v2 path | Task 032 is a completed local `STOP`. GAP-054's capture gate and GAP-055's total-token evaluator are guarded. A live screen of GitHub issues #2–#4 exposed and repaired a BUG-045 false-positive selector recurrence: unrelated process constraints now abstain model-free. This removes harmful context but does not prove accepted-task lift; no eligible independent memory/task pair has been selected. | Select one independently arising task where a pre-existing memory contributes a decision-changing fact absent from source-only evidence. Prove the difference model-free before any capped paired run. |
...
| **P2 — Ship a recoverable customer capability** | Supported hosts receive the proven behavior with clear diagnostics and rollback | BUG-052 provenance is guarded in development; published v2.12.2 and the current local install still predate that contract. Later behavior remains unreleased. | Build an exact-SHA customer candidate, prove host install/repair/known-good reinstall and Recall through that identity, then require explicit release authority. |
...
- BUG-052 is fixed in development, but the current installed runtime still has a
  legacy identity. Installed-candidate evidence and customer promotion must use
  a newly built exact-SHA archive whose payload, runtime manifest, and `doctor`
  report agree; this machine was not silently upgraded.
...
|------|-------|--------|-------------|
| 2026-08-14 | **BUG-045 answer-selection recurrence repaired from a live real-task screen.** Recall was asked whether to implement the three actual open GitHub product issues. It supplied unrelated SDD/developer-etiquette constraints each time instead of abstaining. The v2 selector no longer lets an intrinsic evidence role manufacture a question-specific anchor; role-based delivery now requires at least 20% query-term coverage, while strong direct answers and explicit structural/evaluator anchors remain valid. | One selected memory matched only 3 of 28 task terms (10.7%) and passed solely because it was classified as a constraint. This was a concrete harmful-context and token-cost blocker, not missing architecture. | The exact issue-2 false positive now abstains model-free and removes 167 irrelevant context tokens (188 → 21); a genuinely matched customer constraint, direct factual answer, sealed task-032 fixture, and existing Recall/answer-context contracts remain selected. The 106-test affected set, 470-test fast suite (4 skips, 1 slow deselection), isolated slow two-client proof, and live isolated MCP self-protocol (48/48) pass; scoped Ruff and whitespace checks pass. This removes irrelevant context but is not accepted-task lift. No live store mutation, model run, version, merge, release, or deployment occurred. |
...
| BUG/GAP count tracked | **50 BUG records through BUG-052 + 5 GAPs** | `workspace/ISSUES.md` | Tracked |
