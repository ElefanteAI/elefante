...
>
> **Last Updated:** 2026-05-02 (Hermes audit + corrections; 27 BUGs + 3 GAPs)
...
| GAP-025 | Multi-instance write origin tracking missing — no `source.*` tuple on any memory, and stdio-per-client transport makes two concurrent IDEs a Kuzu single-writer contract violation. Blocks session-intelligence client attribution and cross-IDE reach under concurrent use. | DOCUMENTED — closure planned via `spec-ide-integration-surface.md` Phase A+B (singleton daemon + Source schema) | [ops-memory #15](postmortems/memory.md#issue-15-multi-instance-write-origin-tracking--no-source-attribution-concurrent-writer-data-risk) | Phase B acceptance: two concurrent IDE instances produce distinct `source.instance_id` values with no Kuzu lock contention; see [spec-ide-integration-surface.md § 11](../planning/spec-ide-integration-surface.md) | 0x — first architectural audit |
...
5. If the error surfaces in Python: add a runtime citation pointing to the compendium entry (see [`elefante-orchestrator-agent.md`](../../agents/orchestrator.md) Embedding Rule — case 2)
...
[`../../agents/orchestrator.md`](../../agents/orchestrator.md) — Single operational authority for AI agents developing Elefante. The Five Gates, Embedding Rule, Compendium Trigger Map, Script Routing, Closure Sequence. Read it first.
...
[`../technical/spec-self-protocol.md`](../technical/spec-self-protocol.md) — Authoritative whole-system proof for the live MCP server. Use this when the question is "is Elefante actually running end-to-end?" rather than "did one narrow regression stay fixed?"
...
| **Whole-system proof** | Run `./.venv/bin/python scripts/verify/verify_e2e_tests.py` and use [`../technical/spec-self-protocol.md`](../technical/spec-self-protocol.md) to interpret coverage and exclusions |
...
_Last verified: 2026-05-02 | Elefante v3.0.0 | Hermes audit pass: 7 findings actioned_
