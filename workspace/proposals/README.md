...
| [`session-intelligence.md`](session-intelligence.md) | PARTIALLY IMPLEMENTED IN DEVELOPMENT — one explicit opt-in, metadata-only local value-evidence slice reuses Task Intelligence outcome authority; automatic host instrumentation, broader cards, governed learning, natural field proof, installation, and release remain open | `docs/reference/session-intelligence.md` when shipped |
