...
| [`session-intelligence-activation.md`](session-intelligence-activation.md) | IN PROGRESS — SI-1–SI-3 source and isolated acceptance pass; SI-4/SI-5 exact-package rollback, local activation and installed-task proof remain gated | Existing token/architecture references and HTML dashboard guide describe the source candidate without claiming installed activation; no replacement telemetry design |
