#!/usr/bin/env python3
# ─────────────────────────────────────────────────────────────────────────────
# NAME    : verify_emoji_policy.py
# PURPOSE : Enforce the no-emojis policy across strict docs/source surfaces;
#           the only verifier that checks this codebase etiquette rule.
# WHEN    : After adding or editing any documentation or source files, especially
#           if content was generated by an LLM (which tends to inject emojis).
#           Also run as part of pre-commit hygiene.
# USAGE   : python scripts/verify/verify_emoji_policy.py
# NOTES   : Only checks files in the defined strict surfaces (docs/, src/, scripts/).
#           If a new top-level directory is added with docs, add it to the
#           STRICT_PATHS list in this script.
# ─────────────────────────────────────────────────────────────────────────────
