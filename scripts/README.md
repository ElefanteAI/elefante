...
| `run_task_intelligence_baseline.py` | Runs isolated no-Brief calibration trials from one-commit historical snapshots; requires exact run counts and cumulative total/uncached token caps; separates outcomes by model configuration; parses exactly one complete provider usage event or fails to `unknown`; and can emit one raw-content-free attempt event to an existing, pre-registered local Session Intelligence control workflow. The database, session ID, and workflow ID flags are all-or-nothing and require exactly one pending run. | Establish or resume the calibration baseline after the benchmark verifier passes, or use its bounded evidence adapter inside one explicitly opted-in value workflow. |
| `run_task_intelligence_evaluation.py` | Runs seeded paired no-Brief/Task-Brief or source-only/memory-component trials with identical reasoning rules, v1/v2 profile isolation, source-grounded local retrieval, deterministic sealed-fixture preflight, exact caps, contract-bound schema-v3 outcomes, failure-workspace preservation, and no raw transcripts. One diagnostic condition may attach to an existing pre-registered Session Intelligence control/treatment workflow through the same all-or-nothing evidence flags; exactly one run must be pending. | Calibration only until a behavioral manifest is promotion-ready; inspect every preserved failure before another run, keep v1 as rollback, and run each measured arm against its own pre-registered workflow. |
| `summarize_task_intelligence_evaluation.py` | Pairs profile- and task-contract-isolated outcomes, reports complete-pair acceptance efficiency plus all observed input-and-output token spend, applies clustered 95% confidence and resource limits, and blocks cheap failures, acceptance regressions, stale evidence, or diagnostic-only evidence from promotion. Its value-evidence mode opens Session and Task Intelligence databases read-only to render an honest Value Baseline or one non-persisted matched-task Signal Card. | After paired runs; use the value-evidence flags only for an explicitly opted-in pre-registered workflow. Promotion additionally requires a verified behavioral manifest and independent natural-task evidence. |
