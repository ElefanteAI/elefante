...
| 2    | `verify/verify_mcp_handshake.py` | The CLI proves a real customer JSON-RPC initialize; its reusable doctor path verifies live Recall capability without exposing context. Installer-only `--installation-acceptance` additionally proves one generated project-scoped memory through Recall, removes it, verifies absence, creates a verified backup, and writes a private content-free receipt. |
...
| ------ | ------------ | -------------- |
| `install.py` | Single-entry installer: venv, dependencies, empty DB/schema initialization, strict isolated project registration, required Codex connection, optional non-blocking compatibility previews, disposable installed-Recall acceptance, verified managed baseline backup, and private acceptance receipt. | First-time install, repair, or clean reinstall. |
| `bootstrap_release_bundle.py` | Stable-path wrapper that places a downloadable installer payload at `~/.elefante/app/current`, carries one or more selected project folders into `install.py`, and independently verifies the first-run receipt. | Running a downloadable bundle outside a source checkout. |
...
| `init_databases.py` | Initializes the configured SQLite vector store and Kuzu schema without adding demo or acceptance memories. | After a durable-store reset. |
...
| Script | What it does | When to use it |
| ------ | ------------ | -------------- |
| `restart_elefante.py` | Process-level restart with stale-lock cleanup and optional process-authored PID/version verification. | After `src/` changes that the live server needs to pick up. |
| `backup_elefante_data.py` | Checksum-manifested zip backup engine used by Recover and package transactions; excludes nested recovery archives. Its explicit path options are operator/developer controls, not a customer setup choice. | **Stop Elefante before direct operator use.** Normal customers use Recover's managed directory. |
| `restore_elefante_data.py` | Dry-run-first checksum-verified restore engine used by Recover and package transactions; existing data is moved aside by default. Its explicit archive path is an operator/developer recovery control. | Normal customers select a managed archive in Recover. Direct operator use requires a stopped runtime, preflight, then `--apply`. |
...
| `doctor.py` | Read-only report of runtime health, installer ownership, configured surfaces, certified Codex coverage, non-blocking preview coverage, and installed provenance. Customer readiness requires active installer-owned Recall guidance, verified Codex registration, and live Recall capability with safe annotations and a bounded read-only status; `--json` never exposes host commands, values, or recalled context. | Diagnose readiness before configuring a preview host or after an upgrade, repair, or rollback. |
| `uninstall_elefante.py` | Ownership-safe detachment engine used by the exact official-package uninstall. Dry-run by default; it removes only unchanged Elefante-owned service/host surfaces, preserves modified customer configuration, and leaves app/data removal to the outer verified package transaction. | Detach Elefante-owned integrations without claiming ownership of customer edits. |
...
| `run_product_release_scenarios.py` | Produces private, content-free, exact-artifact receipts for product Scenarios A–F. A/B/C/F first install the supplied package into separate new disposable roots; D runs interruption and a real post-switch failed-update rollback alone with a distinct baseline package; E runs backup, restore, uninstall, and reinstall alone under one disposable root. | Only on a dedicated clean macOS Apple Silicon + Codex acceptance machine. Run `--help`; never point it at customer data or a normal Elefante installation. |
| `verify_product_release_gate.py` | Fails closed unless it can read the exact DMG, pass live macOS signature/Gatekeeper/notarization checks, bind all six private scenario receipts, verify seven distinct native/accessibility evidence files, and bind three unfamiliar-user Scenario A receipts to real first-run receipts with declared zero founder intervention. It creates none of that evidence and cannot machine-prove that a participant is unfamiliar. | After the exact-package scenario runners and human trials produce their receipts; before any product-release approval. It is not yet wired into tagged publication. |
...
| `installer_app.swift` | Native AppKit installer surface requiring detected Codex, exposing other detected hosts only as optional compatibility previews, displaying the managed backup location, and requiring project-folder selection on a fresh install. | Compiled by `build_dmg.py`; not run directly. |
| `installer_gui.py` | Portable Tk fallback with the same fresh-install project-selection contract when Swift is unavailable. | Not run directly in the preferred path. |
...
| `reset_kuzu_nuclear.py` | Backup-and-remove the exact configured Kuzu graph path; rejects broad targets and requires exact confirmation for external storage. | Kuzu corrupted but vector memory must be preserved. |
