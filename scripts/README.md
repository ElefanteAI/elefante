...
| `build_release_client.py` | Builds the macOS Elefante Release Client Candidate 1.0 archive from a strict runtime allowlist: product source, required installer scripts, prebuilt dashboard, and the client lock only. | Validate a customer-only candidate. It is not a public release builder. |
| `verify_release_client.py` | Rejects a client candidate with developer workspace, tests, migration/support utilities, development tools, unexpected files, or an invalid client manifest. | Immediately after building a candidate and in CI before it can be proposed for publication. |
