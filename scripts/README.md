...
| `build_dmg.py` | Builds a branded macOS `.dmg` installer from the installer bundle zip. Compressed UDZO, Elefante volume icon, README, website link. On macOS builders with Swift available, it compiles the native AppKit installer app into `Install Elefante.app`; otherwise it falls back to the legacy Python/Tk launcher path. Optional `--sign` for notarized releases. | In CI (macOS runner) after `build_installer_bundle.py`, or locally when validating DMG packaging. | Platform-specific distribution packaging. Spec: `spec-vision.md` section F. |
| `installer_app.swift` | Native macOS AppKit installer surface for the DMG `.app`. Uses real Cocoa controls for install path selection, progress, status, and live log output while delegating actual installation to `install.sh`. | Not run directly — compiled by `build_dmg.py` into `Install Elefante.app` on macOS builders with Swift available. | Primary macOS customer installer surface. |
| `installer_gui.py` | Legacy macOS Python/Tk installer UI kept as a fallback path when native Swift compilation is unavailable. Delegates to the same `install.sh` authority and preserves the same argument contract. | Not run directly in the preferred path — bundled only as fallback compatibility by `build_dmg.py`. | Compatibility fallback, not the preferred macOS customer surface. |
...
| `installer_app.swift` | Native macOS AppKit installer surface compiled into the DMG `.app` on builders with Swift available. Uses Cocoa controls while delegating installation authority to `install.sh`. | It is the preferred macOS customer installer surface. | **High** (GUI installer) |
| `installer_gui.py` | Legacy Python/Tk compatibility installer surface bundled only when native AppKit compilation is unavailable. | It preserves a fallback path but is no longer the preferred macOS customer surface. | **Medium** (Fallback GUI installer) |
